...
async def echo(bot, update):
    try:
        ...
        await update.reply_text("Use About cmd first /about")
        return
    ...
    if left > 0:
        await update.reply_text(f"Sorry Dude, flood control is active, please wait for {ltime}.", reply_to_message_id=update.message.message_id)
        return
    
    if buy_date == None:
        await message.reply_text(
            f"You can't upload more than {humanbytes(limit)}.\n\n"
    ...
    if remain < int(file.file_size):
        ...
        await message.reply_text(
            ...
            f"File size detected: {humanbytes(file.file_size)}\n"
            f"Used Daily Limit: {humanbytes(used)}\n\n"
            f"You have only {humanbytes(remain)} left on your Account.\n"
            f"If you want to rename large files, please upgrade your plan.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Upgrade 💰💳", callback_data="upgrade")]])
        )
        return
    else:
    ...
    if t_response:
        ...
        if "formats" in response_json:
            # Check if the user has reached the daily limit before showing quality options
            if left > 0:
                ...
                # User has reached the daily limit, prompt to upgrade
                inline_keyboard.append([                 
                    InlineKeyboardButton(
                        "You have reached your daily download limit. Upgrade to get more downloads.", callback_data='upgrade')               
                ])
        else:
